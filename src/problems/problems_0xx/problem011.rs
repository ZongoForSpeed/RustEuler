...
pub fn problem011() -> String {
    ...
    for i in 0..20 {
        if i + 4 < 20 {
            resultat = u32::max(resultat, matrice[i][0] * matrice[i + 1][0] * matrice[i + 2][0] * matrice[i + 3][0]);
        }
        for  j in  0..20 {
            if i + 4 < 20 {
                resultat = u32::max(resultat,
            ...
            if j + 4 < 20 && i + 4 < 20 {
                resultat = u32::max(resultat, matrice[i][j] * matrice[i + 1][j + 1] * matrice[i + 2][j + 2] *
                    matrice[i + 3][j + 3]);
                resultat = u32::max(resultat, matrice[i][j + 3] * matrice[i + 1][j + 2] * matrice[i + 2][j + 1] *
                    matrice[i + 3][j]);
            }
        }
    }
