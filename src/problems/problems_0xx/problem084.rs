...
register_problem!(84, "Monopoly odds", problem084);

pub fn problem084() -> String {
    ...
    // purpose of this problem we are only concerned with cards that order a movement; any
    // instruction not concerned with movement will be ignored and the player will remain on the
    // CC/CH square.
    //
    ...
    // squares will have the lowest probabilities, as 5/8 request a movement to another square,
    // and it is the final square that the player finishes at on each roll that we are interested
    // in. We shall make no distinction between "Just Visiting" and being sent to JAIL, and we
    // shall also ignore the rule about requiring a double to "get out of jail", assuming that they
    ...
    // string.
    let mut plateau: Vec<usize> = vec![0; 40];
    ...
    for _ in 0..10000000 {
        let d1 = rng.random_range(1..=4) as usize;
        let d2 = rng.random_range(1..=4) as usize;
        ...
        match position {
            2 | 17 | 33 => match rng.random_range(1..=16) {
                1 => position = 0,
                2 => position = 10,
                _ => {}
            },
    ...
    for (n, p) in plateau.iter().enumerate() {
        result.push((n, *p));
    }
