...
pub fn problem128() -> String {
    ...
    // so on. The diagram below shows the first three rings.
    //
    // By finding the difference between tile n and each of its six neighbours we shall define PD(n) to be the number
    // of those differences which are prime.
    //
    ...
    //
    // If all of the tiles for which PD(n) = 3 are listed in ascending order to form a sequence, the 10th tile would
    // be 271.
    //
    ...
    for n in 1.. {
        let size = 6 * n;
        if primes.contains(&(size + 1))
            && primes.contains(&(size - 1))
            && primes.contains(&(2 * size + 5))
        {
            count += 1;
            if count == objective {
        ...
        if primes.contains(&(size - 1))
            && primes.contains(&(size + 5))
            && primes.contains(&(2 * size - 7))
        {
            count += 1;
            if count == objective {
